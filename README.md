...
- 可为每首歌曲填写简介，使用独立后台窗口随时修改并开关直播展示
...
1. 导入一首音乐或视频。
2. 点击播放队列中的曲目。
3. 如需补充资料，点击“编辑当前曲目信息”导入封面并填写投稿人、歌曲简介。
4. 在底部“模拟弹幕”中输入：
   - `#01 8.5`
   - `#01评 前奏很惊艳`
...
点击左侧“打开简介后台”可打开独立窗口；进入节目模式时会自动打开。后台可以选择队列中任意曲目，填写简介后点“保存并应用”，勾选或取消“在节目画面展示”会立即生效。节目模式下按 `Ctrl+Shift+D` 可以重新打开后台。OBS/直播姬请使用“窗口采集”选择主节目窗口，后台窗口仅供主播操作。

视频模式仍只显示视频和评论栏，简介不会覆盖视频。简介与开关属于当前播放队列；更新动作会记录到本场 JSONL 存档。
