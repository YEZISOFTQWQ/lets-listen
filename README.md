...
点击左侧“打开直播后台”可打开独立控制窗口；进入节目模式时会自动打开。后台可导入或拖入音视频、选择播放队列中的曲目、播放/暂停、切歌、调整进度与音量，以及调节评论栏透明度、模拟弹幕、管理 B 站直播连接和导出存档。节目模式下按 `Ctrl+Shift+D` 可以重新打开后台。OBS/直播姬请使用“窗口采集”选择主节目窗口，后台窗口仅供主播操作。
